 ...
 *
 */
class ElementPutProcessor {
    
    CodeEmitter e;
    
    public ElementPutProcessor(CodeEmitter e) {
        super();
        this.e = e;
    }
                ...
                if(rawType instanceof Class){
                    ...
                    if(process == false){
                        if (List.class.isAssignableFrom((Class<?>)rawType)) {
                            ...
                            process = buildSet((Class<?>)rawType, pts[0], var);
    ...
    public boolean isSpecial(java.lang.reflect.Type type, java.lang.reflect.Type[] pts){
        if (IMongoBean.class.isAssignableFrom((Class<?>)type)) {
            return true;
        } else if (SizeLimitedMap.class.isAssignableFrom((Class<?>)type)) {
            return true;
    ...
    public boolean processSpecial(java.lang.reflect.Type type, Local var){
        if (IMongoBean.class.isAssignableFrom((Class<?>)type)) {
            e.push(((Class<?>)type).getName());
            e.load_local(var);
            e.invoke(getMethodInfo(MongoBeanUtil.class, "parse", new Class<?>[] { String.class, Map.class }));
            e.store_local(var);
            return true;
        }
        return false;
    ...
    private boolean buildSet(Class<?> clazz, java.lang.reflect.Type elementType, Local var){
        ...
        e.load_local(set);
        e.load_local(value);
        e.invoke(getMethodInfo(Set.class, "add", Object.class));
        e.pop();
    ...
    private boolean buildMap(Class<?> clazz, java.lang.reflect.Type elementType, java.lang.reflect.Type keyType, Local var){
        ...
        if("java.lang.Integer".equals(((Class<?>)keyType).getName())){
            e.checkcast(TypeUtils.parseType("String"));
            e.invoke_static(TypeUtils.parseType("Integer"), TypeUtils.parseSignature("int parseInt(String)"));
            e.invoke_static(TypeUtils.parseType("Integer"), TypeUtils.parseSignature("Integer valueOf(int)"));
        }else if("java.lang.Double".equals(((Class<?>)keyType).getName())){
            e.checkcast(TypeUtils.parseType("String"));
            e.invoke_static(TypeUtils.parseType("Double"), TypeUtils.parseSignature("double parseDouble(String)"));
            e.invoke_static(TypeUtils.parseType("Double"), TypeUtils.parseSignature("Double valueOf(double)"));
        }else if("java.lang.Long".equals(((Class<?>)keyType).getName())){
            e.checkcast(TypeUtils.parseType("String"));
            e.invoke_static(TypeUtils.parseType("Long"), TypeUtils.parseSignature("long parseLong(String)"));
            e.invoke_static(TypeUtils.parseType("Long"), TypeUtils.parseSignature("Long valueOf(long)"));
        }else if("java.lang.Boolean".equals(((Class<?>)keyType).getName())){
            e.checkcast(TypeUtils.parseType("String"));
            e.invoke_static(TypeUtils.parseType("Boolean"), TypeUtils.parseSignature("boolean parseBoolean(String)"));
            e.invoke_static(TypeUtils.parseType("Boolean"), TypeUtils.parseSignature("Boolean valueOf(boolean)"));
        }
        e.store_local(key);
